...
class PlanExecutionModel(Base):
    ...
    retry_of: Mapped[str | None] = mapped_column(
        ForeignKey("plan_executions.id"), nullable=True
    )
...
class StepExecutionModel(Base):
    ...
    execution_id: Mapped[str] = mapped_column(
        ForeignKey("plan_executions.id", ondelete="CASCADE"), index=True
    )
...
class PostgresExecutionRepository:
    ...
    def list_by_plan(self, plan_id: int) -> list[PlanExecution]:
        statement = (
            ...
            .where(PlanExecutionModel.plan_id == plan_id)
            .order_by(PlanExecutionModel.created_at.desc())
        )
        with Session(self._engine) as session:
    ...
    def finish_step(
        ...
        *,
        ...
        statement = (
            ...
            .order_by(StepExecutionModel.id.desc())
        )
        with Session(self._engine) as session:
            model = session.scalar(statement)
            if model is None:
                raise ValueError(
                    f"Step execution {execution_id}/{step_id} was not found"
                )
            model.status = status.value
    ...
    def list_steps(self, execution_id: str) -> list[StepExecution]:
        statement = (
            ...
            .order_by(StepExecutionModel.id)
        )
        with Session(self._engine) as session:
