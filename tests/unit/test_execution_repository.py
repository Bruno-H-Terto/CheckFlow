...
from adapters.postgres.execution_repository import PlanExecutionModel, PostgresExecutionRepository, StepExecutionModel
...
def test_persists_execution_history_results_and_variables() -> None:
    ...
    tables = [cast(Table, PlanExecutionModel.__table__), cast(Table, StepExecutionModel.__table__)]
    ...
    repository.finish_step(execution.id, 10, ExecutionStatus.COMPLETED, status_code=201, latency_ms=3.5, assertions=[{"passed": True}])
